...
graphs, df_valid = joblib.load("dual_graph_dataset_elect.pkl")
train_graphs, test_graphs = stratified_split(graphs, df_valid)

train_loader = DataLoader(train_graphs, batch_size=32, shuffle=True, collate_fn=dual_collate)
test_loader = DataLoader(test_graphs, batch_size=32, shuffle=False, collate_fn=dual_collate)

in_dim = train_graphs[0][0].x.shape[1]
edge_dim = train_graphs[0][0].edge_attr.shape[1]
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

model = TransformerGNN(in_dim=in_dim, hidden_dim=128, edge_dim=edge_dim, dropout=0.15).to(device)
